// Color Math — sRGB, Display-P3, OKLCH, CAM16-UCS, Jzazbz, and APCA helpers.
...
export function hexToY(hex: string): number {
  const [sr, sg, sb] = hexToSrgb(hex);
  // APCA uses a simple 2.4 gamma (not piecewise sRGB TRC) per the SA98G spec
  const Y =
...
export function apcaContrast(fgHex: string, bgHex: string): APCAResult {
  ...
  if (bgY > txtY) {
    ...
    // Uses revBG=0.65, revTXT=0.62 per SA98G spec (not normBG/normTXT)
